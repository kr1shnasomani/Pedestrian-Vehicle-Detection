...
def segment_vehicles(image_path, output_path):
    ...
    # Create a color image with vehicles 
    blue_image = np.zeros((vehicle_mask.shape[0], vehicle_mask.shape[1], 3), dtype=np.uint8)
    blue_image[:, :, 2] = vehicle_mask * 255  

    # Save the segmented image 
    vehicle_image = Image.fromarray(blue_image)
